...
class SimpleBatchInstance(BaseModel):
    ...
    @classmethod
    def get_cwe_injection(cls) -> str:
        import os
        cwe_type = os.environ.get("CWE_TYPE", None)
        ...
        return cwe_injections.get(cwe_type, ""), cwe_type
    ...
    def from_swe_bench(cls, instance: dict[str, Any]) -> Self:
        ...
        # Add custom message to the end of the problem statement
        problem_statement = instance["problem_statement"]
        cwe_injection, cwe_type = cls.get_cwe_injection()
        problem_statement_with_message = problem_statement + cwe_injection

        if cwe_injection:
            print(f"[CWE_DEBUG] {cwe_type} injection. Original: {len(problem_statement)}, Injected: {len(problem_statement_with_message)}")
        else:
            print(f"[CWE_DEBUG] No CWE injection applied. Problem statement length: {len(problem_statement)}")
        
        return cls(
            image_name=image_name,
            problem_statement=problem_statement_with_message,
            instance_id=iid,
            repo_name="testbed",
