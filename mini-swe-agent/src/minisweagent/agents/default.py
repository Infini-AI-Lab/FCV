...
class AgentConfig:
    ...
    cwe_type: Optional[str] = None  # Add CWE type parameter
...
class DefaultAgent:
    def __init__(self, model: Model, env: Environment, *, config_class: Callable = AgentConfig, **kwargs):
        # Check for CWE type from environment variable - prioritize env var over kwargs
        if 'CWE_TYPE' in os.environ:
            kwargs['cwe_type'] = os.environ['CWE_TYPE']
    ...
    def get_cwe_injection(self) -> str:
        """Get CWE-specific injection text based on config."""
        ...
        return cwe_injections.get(self.config.cwe_type, "")
    ...
    def run(self, task: str, **kwargs) -> tuple[str, str]:
        ...
        # Apply CWE injection to the task content if CWE type is set
        if self.config.cwe_type:
            cwe_injection = self.get_cwe_injection()
            # Inject CWE content after the task (PR description)
            task_with_injection = task + cwe_injection
            self.extra_template_vars |= {"task": task_with_injection, **kwargs}
            # Debug info - remove this after verification
            print(f"[CWE_DEBUG] Applied {self.config.cwe_type} injection. Original: {len(task)}, Injected: {len(task_with_injection)}")
        else:
            self.extra_template_vars |= {"task": task, **kwargs}
            # Debug info - remove this after verification
            print(f"[CWE_DEBUG] No CWE injection applied. Task length: {len(task)}")
